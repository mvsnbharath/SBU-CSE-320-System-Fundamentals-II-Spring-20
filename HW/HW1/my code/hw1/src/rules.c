...
SYMBOL *new_rule(int v) {
    ...
    SYMBOL* newly_created_rule = new_symbol(v,NULL);
    newly_created_rule->rule = newly_created_rule ;
    newly_created_rule-> next = newly_created_rule ;
    newly_created_rule-> prev = newly_created_rule ;
    *(rule_map+v)= newly_created_rule;
    debug("New rule (%ld) for %d",SYMBOL_INDEX(newly_created_rule),v);
    return newly_created_rule;
}
...
void add_rule(SYMBOL *rule) {
    if (main_rule==NULL){
        ...
        debug("Add rule (%ld)",SYMBOL_INDEX(rule));
        return;
    }else{
...
SYMBOL *ref_rule(SYMBOL *rule) {
    debug("Reference rule [%ld] for [%d]]: %d -> %d",SYMBOL_INDEX(rule),rule->value,
...
void unref_rule(SYMBOL *rule) {
    debug("Derference rule [%ld] for [%d]]: %d -> %d",SYMBOL_INDEX(rule),rule->value,
        rule->refcnt, (rule->refcnt)-1);
    rule->refcnt = rule->refcnt - 1;
    return ;
}
