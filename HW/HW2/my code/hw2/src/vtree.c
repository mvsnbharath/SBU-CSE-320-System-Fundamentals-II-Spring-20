...
#define SIZE_IN_BYTES 1024/BLOCKSIZE
...
/* open subdirectory */
    ...
    if (sort) {
        tmp_RD = head;
        while (tmp_RD) {
           // debug("Start: %s",NAME(tmp_RD->entry));
            tmp1_RD = tmp_RD->fptr;
            while (tmp1_RD) {
                //debug("Comparing: %s, %s",NAME(tmp_RD->entry),NAME(tmp1_RD->entry));
...
/* print totals */
    ...
    tmp_RD = head;
    while (tmp_RD) {
        struct RD_list* temp_copy = tmp_RD;
        tmp_RD = tmp_RD->fptr;
        free(temp_copy);
    }
#endif
